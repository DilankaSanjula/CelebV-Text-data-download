...
def download(video_path, ytb_id, proxy=None):
    ...
    try:
        link = f"https://www.youtube.com/watch?v={ytb_id}"
        video = YouTube(link)
        #file_size = video.streams.get_highest_resolution().filesize
        stream = video.streams.get_by_itag(137)
        #print(f"YT Video ID: {ytb_id} | YT Video size: {file_size}")

        if stream and not os.path.exists(video_path):
            ...
            print("Stream with format code 137 not found.")
            
    except Exception as exp:
        print("skipping cannot get yt video file size", exp)
...
if __name__ == '__main__':
    ...
    for vid_id, save_vid_name, time, bbox in load_data(json_path):
        ...
        raw_vid_path = f"downloaded_celebvhq/raw/{vid_id}.mp4"
